...
public class FavoritesPresenter extends MvpPresenter<FavoritesMvp> {
    ...
    void getFavoriteMovies(int accountId, String sessionId) {
        ...
        // todo Sorts in another class
        disposable = service.getFavoriteMovies(accountId, BuildConfig.TMDB_API_KEY, sessionId, TmdbConfigKt.en_US, "created_at.asc", page).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
            .subscribe(response -> {
