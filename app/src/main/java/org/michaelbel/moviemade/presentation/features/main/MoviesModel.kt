...
class MoviesModel @AssistedInject constructor(
    ...
    val toolbarTitle = MutableSharedFlow<Any>()
    val toolbarSubtitle = MutableSharedFlow<String>()
    ...
    init {
        viewModelScope.launch {
            toolbarTitle.emit(
                when {
                    list != null && list == Movie.SIMILAR -> R.string.similar_movies
                    list != null && list == Movie.RECOMMENDATIONS -> R.string.recommendations
                    list != null && list == Movie.FAVORITE -> R.string.favorites
                    list != null && list == Movie.WATCHLIST -> R.string.watchlist
                    keyword != null -> keyword.name.toString()
                    else -> toString()
                }
            )
            if (movie != null) {
                toolbarSubtitle.emit(movie.title.toString())
            }
        }
        load()
    }
