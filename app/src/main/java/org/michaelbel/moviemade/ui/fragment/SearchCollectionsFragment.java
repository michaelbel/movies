    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        recyclerView.setOnItemClickListener((view, position) -> {
            if (view instanceof CollectionView) {
                Collection collection = (Collection) adapter.getCollections().get(position);
    ...
    public void showResults(List<TmdbObject> results, boolean firstPage) {
        if (firstPage) {
            ...
            if (AndroidUtilsDev.searchResultsCount()) {
                TabLayout.Tab tab = activity.binding.tabLayout.getTabAt(SearchActivity.TAB_COLLECTIONS);
                if (tab != null) {
