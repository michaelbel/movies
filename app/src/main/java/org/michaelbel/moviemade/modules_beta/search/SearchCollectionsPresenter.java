...
public class SearchCollectionsPresenter extends MvpPresenter<SearchMvp> {
    ...
    public void search(String query) {
        ...
        Observable<CollectionResponse> observable = service.searchCollections(BuildConfig.TMDB_API_KEY, Url.en_US, query, page).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread());
    ...
    public void loadNextPage() {
        SEARCH service = ApiFactory.createService2(SEARCH.class);
        Observable<CollectionResponse> observable = service.searchCollections(BuildConfig.TMDB_API_KEY, Url.en_US, currentQuery, page).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread());
        disposables.add(observable.subscribeWith(new DisposableObserver<CollectionResponse>() {
            @Override
            public void onNext(CollectionResponse response) {
                List<TmdbObject> results = new ArrayList<>(response.collections);
